...
public class StreamPraktiniai {
    ...
    public static List<String> getFirstNames(List<String> names){
       return names.stream().map(x -> x.split(" ")[0]).toList();
    }

    public static List<String> getDistinctLetters(List<String> names){
       return names.stream().flatMap(x -> x.chars().mapToObj(c -> String.valueOf((char) c))).distinct().toList();
    }


    public static String separateNamesByComma(List<User> users){
        return users.stream().map(User::getName).map(x-> x.split(" ")[0]).collect(Collectors.joining(", "));
    }
    ...
    public static List<Integer> generateFirst10PrimeNumbers(){
        ...
        for(int i = 2; primeNums.size() < 10; i++) {
            if(isPrime(i)) {
                primeNums.add(i);
            }
        } return primeNums;
    ...
    public static List<Integer> generate10RandomNumbers(){
        ...
        return IntStream.range(0,10).map(i -> random.nextInt(1000000000)).boxed().toList();
    }
    ...
    public static Map<Boolean, Long> countGender(List<User> users){
       return users.stream().collect(groupingBy(User::isMale,Collectors.counting()));
    }
